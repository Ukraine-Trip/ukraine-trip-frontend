...
export const MapComponent: React.FC<{ itinerary?: ItineraryPoint[] }> = ({
  ...
  return (
    <MapWrapper style={{ height: '100%', width: '100%', position: 'absolute', top: 0, left: 0 }}>
      <MapPageLayout>

        {/* ══════════════════════════════════════════
            SIDEBAR — 30% ліворуч (на мобайлі — зверху)
        ══════════════════════════════════════════ */}
        <RouteSidebar $collapsed={!sidebarOpen}>
          ...
          {tripMeta ? (
                    ...
                    {tripMeta.waypoints.map((wp, i) => (
                      <li
                        ...
                        style={{
                          ...
                          padding: '8px 10px',
                          borderRadius: '8px',
                          background: i === 0 ? '#f0f7f4' : i === tripMeta.waypoints.length - 1 ? '#f7f0f0' : 'transparent',
                        }}
                      >
                        <span style={{
                          width: '22px',
                          height: '22px',
                          borderRadius: '50%',
                          background: i === 0 ? '#2e7d5a' : i === tripMeta.waypoints.length - 1 ? '#c0392b' : '#e8e8e8',
                          color: i === 0 || i === tripMeta.waypoints.length - 1 ? '#fff' : '#555',
                          display: 'flex',
                          alignItems: 'center',
            ...
            /* Заглушка коли маршрут не вибрано */
            <div style={{
              flex: 1,
              display: 'flex',
              ...
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#ddd" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: '16px' }}>
                <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z" />
                <circle cx="12" cy="9" r="2.5" />
              </svg>
              <p style={{ margin: 0, fontSize: '14px', lineHeight: 1.5 }}>
                Select a route from the<br />home page to see details
              </p>
            </div>
          )}
        </RouteSidebar>
        ...
        {/* ══════════════════════════════════════════
            MAP AREA — 70% праворуч (на мобайлі — знизу)
        ══════════════════════════════════════════ */}
        <MapArea>
          ...
          >
            <svg
              ...
              style={{ transition: 'transform 0.3s', transform: sidebarOpen ? 'rotate(0deg)' : 'rotate(180deg)' }}
            >
              <polyline points="7,1 3,5 7,9" />
            </svg>
          </button>

          {/* ── ПЕРЕМИКАЧ ОПТИМІЗАЦІЇ (верхній правий) ── */}
          <div
          ...
          {/* ── КНОПКА ШАРІВ (під перемикачем, правий край) ── */}
          <div
          ...
          {/* ── ПАНЕЛЬ ШАРІВ (відкривається вліво від кнопки) ── */}
          {layerPanelOpen && (
            <div
              style={{
                position: 'absolute',
          ...
          >
            ...
            <UserLocation ctrlTop={CTRL_TOP + 52 + 56} />
